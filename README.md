<h1 align="center">Hey 👋 I'm Amol Satsangi</h1>
<h3 align="center">C++ Developer | Electrical Engineering (with CS) | IEEE Chair</h3>

<p align="center">
  <em>Building efficient systems at the intersection of software, hardware, and engineering.</em>
</p>

---
![Stats](https://github-readme-stats.vercel.app/api?username=Amol-Satsangi1266&show_icons=true&theme=tokyonight)

![Streak](https://streak-stats.demolab.com?user=Amol-Satsangi1266&theme=tokyonight)

---
💻 **C++ & Backend-Oriented Developer** with strong foundations in  
Data Structures, OOP, Linux, and System-Level Programming.

🎓 **B.Tech (Electrical Engineering with CS)**  
📍 Dayalbagh Educational Institute, Agra | CGPA: 8.1  

⚡ **Ex-Chair — IEEE Student Branch, DEI**  
Leading research, innovation, and technical initiatives.

---

### 🔥 What I Work On
- 🧠 **Custom STL & Algorithms in C++** (memory management, performance-focused)
- 🧰 **Console-based C++ Systems** (OOP, STL, file handling)
- 🔌 **Arduino + Embedded Projects** (medical & assistive tech)
- 🏭 **Industry Exposure** — R&D @ Thermal Power Plant & Product Systems @ BlueStar

---

### 🛠 Tech Arsenal
**Languages:** C, C++, Java  
**Core CS:** DSA, OOPs, Git, GitHub, Linux  
**Web:** HTML, CSS, JavaScript, Tailwind CSS  
**Domains:** Backend Systems • Embedded Logic • Engineering Software

---

### 🏆 Highlights
- 🥇 Finalist — **IIT Delhi Entrepreneurship Competition**
- 🧠 Finalist — **IIT Delhi Be’Con (Find Your Co-Founder)**
- 🛡 Organizer — **Cybersecurity Workshop**
- 🇮🇳 Delegate — **IEEE AICCSYC 2024**
- 🚀 SIH 2024 & 2025 — Internal Qualifier

---

### 📌 Philosophy
> *I know and I believe that I give my 100% to make a problem easily understanbale and solvable!*

---

📫 **Reach Me:**  
📧 amolpankaj14@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/amol-satsangi-a35a212ab/)  
🐙 [GitHub](https://github.com/Amol-Satsangi1266)
